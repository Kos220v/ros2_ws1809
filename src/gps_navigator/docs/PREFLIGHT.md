...
5. **Курс по GPS (антидрейф).** Узел `gps_heading` публикует направление
   фактического перемещения в `/gps/heading`. Проверка: проезжайте прямо
   ~10 м и посмотрите `ros2 topic echo /gps/heading` — yaw должен быть
   устойчивым и соответствовать направлению движения (ENU: север = +1.57).
   Это наблюдение не даёт дрейфу курса IMU накапливаться в `map`, пока
   робот едет; на стоянке топик молчит — это норма.

6. **Проверка истинного севера (для GPS).** Проедьте 10 м строго на север.
...
7. **Дрейф курса в покое.** Курс берётся только из кватерниона IMU (ROS
   гироскоп нигде не интегрирует), поэтому «плывущий» угол лечится только
   калибровкой платы. Измерение (робот неподвижен, 60 с):

   ```bash
   ros2 run imu_stm32_bridge gyro_drift_check --ros-args -p duration_s:=60.0
   ```

   Норма: < 1 °/мин. Вердикт `GYRO_BIAS` — дрейф совпадает с интегралом `wz`:
   калибровать гироскоп (`gyro_calib` + `save_flash`, робот неподвижен ~2 с).
   Вердикт `MAG` — калибровать магнитометр (п. 2). Подробно:
   `src/imu_stm32_bridge/docs/CALIBRATION.md`. Учтите: `zero_yaw` дрейф
   не лечит, он только сбрасывает текущее значение курса.
...
2. Когда робот едет сам — переведите тумблер в положение 1 (MANUAL) и
   дайте газ: робот обязан мгновенно перейти под ручное управление.
   Миссия при этом отменяется (`gps_mission` запоминает точку).
3. Переведите тумблер обратно в положение 2 (AUTO) — маршрут продолжится
   сам с той же точки (`start_on_auto: true` в `mission_params.yaml`).
